...
#      #or not product_review.user==current_user request.user.is_superuser
#     if not request.user.is_superuser:
#         messages.error(request, 'Sorry, only store owners can do that.')
#         print(current_user)
...
@login_required
def delete_review_author(request, review_id):
    """ Delete a review to a product """
    product_review = get_object_or_404(Review, pk=review_id)
    current_user = request.user
     #or not product_review.user==current_user request.user.is_superuser
    if not product_review.user==current_user:
        messages.error(request, 'Sorrry, only store owners can do that.')
        print(current_user)
        print(product_review.user)
        return redirect(reverse('home'))
    ...
    return render(request, 'delete_review_authors.html')
